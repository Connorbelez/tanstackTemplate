...
- [x] T-001: Add `reserveShares` to [convex/ledger/mutations.ts](convex/ledger/mutations.ts) as an `internalMutation` using `reserveSharesArgsValidator` and the existing ledger helpers/import conventions.
...
- [x] T-003: Implement the reservation write path in [convex/ledger/mutations.ts](convex/ledger/mutations.ts): resolve seller and buyer POSITION accounts, call `postEntry` with `SHARES_RESERVED` before mutating pending fields, increment `seller.pendingCredits` and `buyer.pendingDebits`, insert the `ledger_reservations` row, backfill `journalEntry.reservationId`, and return `{ reservationId, journalEntry }`.
...
## Phase 3: Verify
- [ ] T-008: Run `bunx convex codegen`, `bun check`, `bun typecheck`, and `bun test` from [repo root](.) after implementation.
