...
- [x] T-016: In `governedTransitions.ts`, create `executeEffect` as an `internalMutation`. Args: `{ entityId, journalEntryId, effectName }`. Inserts an entry to `demo_gt_effects_log` with status "completed" directly (simulating instant success). This simulates the fire-and-forget pattern — in production, effects would be `internalAction`s calling external APIs. (F-5, REQ-6)
