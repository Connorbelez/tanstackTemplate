...
All admin-facing listing and mortgage surfaces changed by this phase MUST remain inside the current dashboard shell:

- existing global sidebar remains primary navigation,
- breadcrumbs appear in the header,
- listing-projection controls/actions live in the page header or in-page cards,
- projection data is rendered as normal dashboard cards/tables, not as a separate microsite.

Recommended breadcrumb patterns:

- mortgage detail linkage view: `Admin / Mortgages / {mortgageId}`
- listing detail/admin view: `Admin / Listings / {listingId or title}`

This phase SHOULD reuse the repo’s existing listing detail chrome if it already exists, but it MUST retrofit that chrome to the projection-vs-curation information architecture defined below.

## Admin mortgage-detail linkage UI

The mortgage detail page gains a `Linked listing` card or section. This is a compact navigational/control surface, not the full listing editor.

The card SHOULD show:

- linked listing title or fallback ID,
- listing status (`draft` expected immediately after origination),
- source badge `Mortgage-backed projection`,
- last projection refresh timestamp if available,
- action to open listing detail.
...
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│ Linked listing                                                                              │
│ Listing: 12 King St opportunity                                                             │
│ Status: [Draft]  Source: [Mortgage-backed projection]                                       │
│ Economics/property fields are projection-owned; curated fields stay on                      │
│ listing detail.                          [Open listing]                                     │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```

## Listing detail/admin screen design

The listing detail page MUST make the projection model visually obvious. The operator should immediately understand which fields are read-only projector-owned truth and which fields remain editable curation.

Recommended top-of-page pattern:

- breadcrumbs,
- listing title,
- listing status chip,
- data-source chip `Mortgage-backed projection`,
- linked mortgage link in the header metadata row,
- optional `Refresh projection` action if the repo exposes that operationally.

Immediately beneath the header, show a source-of-truth banner:

```text
This listing is generated from a mortgage-backed projection.
Economics, property facts, appraisal summary, and public docs are canonical projection fields.
Only curated marketplace fields are editable here.
```

### Recommended two-column layout

Left/main column: projector-owned, read-only cards.

- Economics
- Property facts
- Appraisal summary
- Public docs compatibility section

Right/sidebar column: curated editable controls.
...
- SEO slug
- Admin notes
- Publish/delist controls if those already exist in the repo

ASCII mockup:

```text
┌────────────────────┬────────────────────────────────────────────────────────────────────────┐
│ Admin              │ Breadcrumbs: Admin / Listings / 12 King St opportunity                 │
│ Mortgages          │ 12 King St opportunity  [Draft] [Mortgage-backed projection]           │
│ Listings           │ Linked mortgage: M-2031              [Refresh projection]              │
│ Deals              │ ───────────────────────────────────────────────────────────────────────│
│                    │ [Projection read-only. Curated fields remain editable on the right.]   │
│                    │ ┌─────────────────────────────┐ ┌──────────────┐                       │
│                    │ │ Economics (read-only)       │ │ Curated      │                       │
│                    │ │ Principal $850k · pay $8.5k│ │ Title, descr │                        │
│                    │ │ Rate 11.5% · LTV 62%        │ │ Images, SEO  │                       │
│                    │ ├─────────────────────────────┤ │ Publish      │                       │
│                    │ │ Property facts              │ └──────────────┘                       │
│                    │ │ 12 King St • residential    │                                        │
│                    │ ├─────────────────────────────┤                                        │
│                    │ │ Appraisal summary           │                                        │
│                    │ │ $1.37M as-is • 2026-04-10   │                                        │
│                    │ ├─────────────────────────────┤                                        │
│                    │ │ Public documents            │                                        │
│                    │ │ 0 docs in compatibility     │                                        │
│                    │ └─────────────────────────────┘                                        │
└────────────────────┴────────────────────────────────────────────────────────────────────────┘
```

## Field-editability treatment

Projection-owned values SHOULD be rendered with one or more of the following cues:

- lock icon,
- muted input styling without edit affordance,
- label text like `Projection-owned`,
- helper text explaining the canonical source (mortgage/property/valuation).

Curated fields SHOULD remain truly editable and visually distinct:

- standard editable inputs,
- save action in the curated section,
- no accidental inline edit affordance on projector-owned cards.

The UI MUST NOT let an operator directly edit:

- projected economics,
- projected property facts,
- appraisal summary,
- projected public-doc compatibility state.

## `monthlyPayment` rendering rule in UI

The UI MUST render `monthlyPayment` together with `paymentFrequency` every time. Even though the field name is legacy, the visual representation MUST avoid implying a normalized monthly-equivalent amount.

Recommended label patterns:

- `Payment amount`
- `Payment cadence`
- or a single line such as `$8,500 / bi-weekly`

Avoid a lone label `Monthly payment` when the cadence is not monthly.

## Public-doc compatibility section in this phase

Until phase 6 lands, the listing detail page MAY show a compatibility-driven public-doc section. The UI MUST present it as read-only projection output, not as an authoring control.

Recommended empty state:

- `No public origination docs projected yet.`
- helper text that these documents are driven by mortgage-owned public document blueprints, not by listing uploads.

## Generic listing-create path narrowing UX

Because this phase narrows the generic mortgage-backed listing create path, the UI SHOULD remove or de-emphasize any `Create listing` affordance that would create a `mortgage_pipeline` listing outside the projector.

If legacy route access still exists for direct navigation, show a blocked operational page inside the dashboard shell:

```text
┌─────────────────────────────────────────────────────────────────────────────────────────────┐
│ Mortgage-backed listings are not created here                                               │
│ Use Admin → Originations for mortgage-backed listings (canonical path).                     │
│ Non-mortgage listing flows may still exist elsewhere in the product.                        │
│                                    [Go to originations]                                     │
└─────────────────────────────────────────────────────────────────────────────────────────────┘
```

Do not silently allow a duplicate mortgage-backed listing to be created.

## Core user flows the UI MUST support

### Flow A — inspect projected truth

1. Operator commits origination.
2. Operator opens the linked listing from the mortgage detail page.
3. Listing detail page shows projector-owned economics/property/appraisal fields and curated fields.
4. The operator can inspect projection truth without confusing it for editable curation.

### Flow B — edit curation safely

1. Operator edits title/description/images/SEO.
2. Saves changes.
3. A later projection refresh occurs.
4. Curated fields remain intact while projection-owned cards refresh from canonical domain records.

### Flow C — inspect payment display without field-name confusion

1. Operator opens the economics card.
2. Sees payment amount paired with cadence.
3. Understands that the underlying field name may be `monthlyPayment`, but the UI does not misrepresent non-monthly cadences.

## Interaction and visual-behavior rules

- Prefer a persistent explanatory banner over hidden tooltip-only explanations for the projection model.
- Do not allow inline editing of projection-owned values even if the component library makes read-only inputs easy to reuse.
- Linked mortgage and linked listing navigation SHOULD be high visibility; operators will frequently pivot between those screens.
- Keep the listing in `draft` after origination unless an explicit publish flow elsewhere changes it.
- If the projection refresh fails, show a clear error state; do not hide it by continuing to display stale fields as if they were current.

## Merge-safe UI ownership notes

Later phases extend only the public-doc section of the listing detail page. They MUST NOT redesign the projection-vs-curation split introduced here. This split is part of the architectural stance and should become visually stable starting now.
