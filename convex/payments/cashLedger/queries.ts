...
// ── Available Balance (Pre-Validation Gate) ────────────────────
...
export const getAvailableLenderPayableBalance = cashLedgerQuery
	...
	.handler(async (ctx, args) => {
		...
		// TODO(ENG-UPR): Deduct in-flight outbound transfers from transferRequests
...
/**
 * Internal query variant for use by disbursementGate.ts.
 * Returns plain numbers instead of bigints.
 */
export const getAvailableLenderPayableBalanceInternal = internalQuery({
	...
	handler: async (ctx, args) => {
		...
		// TODO(ENG-UPR): Deduct in-flight transfers when transferRequests schema is ready
		const inFlightAmount = 0n;

		return {
			grossBalance: safeBigintToNumber(grossBalance),
			inFlightAmount: safeBigintToNumber(inFlightAmount),
			availableBalance: safeBigintToNumber(grossBalance - inFlightAmount),
		};
	},
});
