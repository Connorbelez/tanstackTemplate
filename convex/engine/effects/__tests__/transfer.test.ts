...
describe("publishTransferConfirmed effect", () => {
	...
	it("preserves ledger posting when bridged reconciliation throws", async () => {
		const t = createTransferHarness();
		const seeded = await seedMinimalEntities(t);
		const obligationId = await createObligation(t, {
			mortgageId: seeded.mortgageId,
		...
		const reconcileSpy = vi
			...
			.mockRejectedValueOnce(new Error("transition exploded"));
	...
	it("preserves ledger posting when integrity-defect logging throws", async () => {
		const t = createTransferHarness();
		const seeded = await seedMinimalEntities(t);
		const obligationId = await createObligation(t, {
			mortgageId: seeded.mortgageId,
		...
		const auditLogModule = await import("../../../auditLog");
		const reconcileSpy = vi
			.spyOn(
				collectionAttemptReconciliation,
				"reconcileAttemptLinkedInboundSettlement"
		...
		const auditLogSpy = vi
			.spyOn(auditLogModule.auditLog, "log")
			.mockRejectedValueOnce(new Error("audit sink exploded"));
