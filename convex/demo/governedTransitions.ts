...
export const transition = convex
	.mutation()
	.input({
...
export const createEntity = convex
	.mutation()
	.input({
		label: v.string(),
		loanAmount: v.number(),
...
export const seedEntities = convex
	.mutation()
	.handler(async (ctx) => {
...
export const runFullLifecycle = convex
	.mutation()
	.handler(async (ctx) => {
		// 1. Create entity
...
export const getEntity = convex
	.query()
	.input({ entityId: v.id("demo_gt_entities") })
...
export const getJournalStats = convex
	.query()
	.handler(async (ctx) => {
		const entries = await ctx.db.query("demo_gt_journal").collect();
		let transitioned = 0;
...
export const getValidTransitions = convex
	.query()
	.input({ entityId: v.id("demo_gt_entities") })
	.handler(async (ctx, { entityId }) => {
		const entity = await ctx.db.get(entityId);
...
export const getEffectsLog = convex
	.query()
	.input({
		entityId: v.optional(v.id("demo_gt_entities")),
	})
...
export const getMachineDefinition = convex
	.query()
	.handler(async () => {
