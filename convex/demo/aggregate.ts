...
export const addScore = authedMutation
	.input({ player: v.string(), score: v.number() })
...
export const removeScore = authedMutation
	.input({ id: v.id("demo_aggregate_scores") })
	.handler(async (ctx, args) => {
...
export const listScores = authedQuery
	.handler(async (ctx) => {
		return await ctx.db
